...
protocol APILBankProtocol {
    func getData(completion:@escaping ((_ data:Bool,_ err: BKDNAPIError?) -> ()))
}

class APILBank: APILBankProtocol {
    let api = API()
    func getData(completion:@escaping ((_ data:Bool,_ err: BKDNAPIError?) -> ())) {
        let host   = BKDNConfig.getHost(url: BKDNUrl.PBankList)
...
extension APILBank {
    ...
    func converTOArr(data: [Dictionary<String,Any>]){
        ...
        for d in data{
            let a = d["bank_name"] as? String ?? ""
            arr.append(["title": a])
        }
        MOList.shared?.bank = arr
    }
}
